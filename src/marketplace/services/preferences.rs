...
impl PreferencesService {
    /// Construct a preferences service from explicit values (used mainly in tests).
    pub fn new(prefs: UserPreferences) -> Self {
        Self { prefs, path: None }
    }

    /// Load preferences from the configuration directory, creating a persisted default if missing.
    ...
    /// Persist current preferences to disk (no-op when the service was created without a path).
    ...
    /// Update preferences and persist the change if the service is backed by disk.
