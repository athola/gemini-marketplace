//! Axum-based fixture server for marketplace integration tests.
//!
//! Serves static catalog and manifest JSON files from `tests/data/marketplace`
//! while allowing tests to toggle GitHub-style rate limiting headers. The server
//! binds to localhost on a random port and exposes helpers to retrieve the base
//! URL and control rate-limit behaviour.
...
/// Embedded Axum server exposing marketplace fixtures.
...
impl FixtureServer {
    /// Launch a fixture server bound to localhost serving files from the
    /// `tests/data/marketplace` directory.
    ...
    /// Base URL that tests can use to fetch fixtures.
    pub fn base_url(&self) -> &str {
        &self.base_url
    }

    /// Toggle rate limiting behaviour. When enabled, requests return HTTP 403
    /// with GitHub-style rate limit headers. `reset_in` controls the reset time.
    ...
    /// Convert a logical fixture path (e.g., `curated/index.json`) into a full
    /// URL served by this fixture server.
...
async fn serve_fixture(
    ...
    if state.control.is_rate_limited() {
        ...
        if let Some((reset, remaining)) = state.control.rate_limit_headers().await {
            ...
            headers.insert(
                "x-ratelimit-reset",
                header::HeaderValue::from_str(&reset).unwrap_or_else(|_| header::HeaderValue::from_static("0")),
            );
            headers.insert(
