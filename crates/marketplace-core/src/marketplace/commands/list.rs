//! Command for listing marketplace extensions.
...
/// Options for listing marketplace extensions.
#[derive(Debug, Clone)]
...
/// A serializable response for CLI list commands.
#[derive(Serialize)]
...
pub async fn execute(opts: ListOptions) -> Result<()> {
    ...
    if let Some(keyword) = opts.search.as_deref() {
        Telemetry::global().record_search(keyword);
    }
