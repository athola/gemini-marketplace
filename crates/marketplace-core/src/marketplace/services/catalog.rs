//! The catalog service, which is responsible for aggregating extensions across sources, applying filtering, and translating results for CLI/API consumers.
...
mod tests {
    ...
    use crate::marketplace::cache::{init, store::CacheStore};
    ...
    async fn list_returns_cached_entries_on_rate_limit() {
        ...
        init::ensure_layout(&config).expect("dirs");
