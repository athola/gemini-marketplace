//! The marketplace source fetcher, which is responsible for retrieving manifests, respecting caching, and handling GitHub rate limits.
...
use crate::marketplace::ingest::validation::build_validation_summary;
use crate::marketplace::models::domain::{
    Extension, ExtensionId, InstallStatus, MarketplaceSource, RateLimitWindow, ValidationStatus,
    ValidationSummary,
};
...
/// Fetches manifests from marketplace sources, respecting the cache TTL and GitHub rate limits.
    ...
    pub async fn sync_source(&self, source: &MarketplaceSource) -> Result<Vec<Extension>> {
        ...
        if let Some(snapshot) = self.cache.load(&source.slug)? {
            if snapshot.entry.expires_at > SystemTime::now() {
                Telemetry::global().record_cache_hit(&source.slug);
        ...
        if response.status().as_u16() == 403 {
            if let Some(window) = Self::extract_rate_limit(&source.slug, &response) {
                Telemetry::global().record_rate_limit_wait(&source.slug);
        ...
        for manifest_url in manifest_urls {
            ...
            if matches!(summary.semantic_status, ValidationStatus::Failed) {
                let reason = summary
                    .errors
                    .iter()
                ...
                return Err(MarketplaceError::InvalidManifest {
                    repository: manifest_url_owned,
                    reason,
                });
            }
            let checksum = format!("{:x}", Sha256::digest(manifest_body.as_bytes()));
            extensions.push(Self::to_extension(
                source, &manifest, ttl, checksum, summary,
            )?);
        }

        self.cache.save(&source.slug, &extensions, etag, ttl)?;
        Telemetry::global().record_cache_miss(&source.slug);
        Ok(extensions)
    }

    pub fn cached_extensions(&self, source_slug: &str) -> Result<Option<Vec<Extension>>> {
        let snapshot = self.cache.load(source_slug)?;
        if snapshot.is_some() {
            Telemetry::global().record_cache_hit(source_slug);
        }
        Ok(snapshot.map(|snapshot| snapshot.extensions))
    }
