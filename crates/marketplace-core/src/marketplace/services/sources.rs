//! The source registry service, which is responsible for managing marketplace sources.
//!
//! This service handles the persistence, validation, and lifecycle of marketplace sources, including the default curated source.
...
impl SourcesService {
    ...
    pub fn new() -> Result<Self> {
        ...
        // Initialize with the default source if the sources file doesn't exist.
    ...
    /// Initializes the sources registry with the default curated source.
    ...
    /// Loads the sources registry from disk.
    fn load_registry(&self) -> Result<SourcesRegistry> {
        if !self.sources_path.exists() {
            // Return an empty registry if the file doesn't exist.
    ...
    /// Saves the sources registry to disk.
    ...
    /// Returns the default curated source.
    ...
    /// Removes a marketplace source by its slug.
    ///
