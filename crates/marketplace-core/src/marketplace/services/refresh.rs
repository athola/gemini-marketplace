//! A refresh scheduler service that is backed by a persisted job queue.
...
impl RefreshService {
    /// Creates a refresh service that is rooted in the provided configuration directory.
    ...
    /// Queues (or replaces) a refresh job and persists the queue to disk.
    pub fn queue_refresh(&self, job: RetryJob) -> Result<()> {
        ...
        Telemetry::global().record_refresh_queue_depth(guard.len());
        Ok(())
    }

    /// Returns all queued jobs, from oldest to newest.
    ...
    /// Drains jobs that are scheduled at or before the provided timestamp.
    ...
    /// Persists the current queue state without modifying it.
