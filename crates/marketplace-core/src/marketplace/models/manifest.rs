//! A raw manifest representation that is parsed from `gemini-extension.json`.
//!
//! This module provides helpers to load and validate manifests while normalizing fields that must conform to marketplace expectations. Validation collects warnings instead of failing fast so that callers can decide whether to skip or continue processing a source.
...
impl ExtensionManifest {
    /// Loads a manifest from disk and returns the parsed manifest and any validation warnings.
