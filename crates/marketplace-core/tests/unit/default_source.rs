//! Unit tests for the default curated source configuration.
//!
//! This module validates that the default marketplace source is correctly initialized, persisted, and validated.
...
fn test_default_curated_source_structure() {
    // Verifies that the default curated source has the correct structure.
...
fn test_default_source_url_validation() {
    // Ensures that the default source URL is exactly as specified.
...
fn test_sources_service_initializes_with_default() {
    // Verifies that SourcesService can be initialized.
...
fn test_sources_service_loads_default_source() {
    // Verifies that `get_default_source` returns the curated source.
    let service = SourcesService::new();
...
fn test_sources_service_lists_includes_default() {
    // Verifies that `list_sources` includes the default source.
    let service = SourcesService::new();
...
fn test_default_source_cannot_be_removed() {
    // Verifies that the default source cannot be removed.
    let service = SourcesService::new();
...
fn test_default_source_can_be_disabled() {
    // Verifies that users can disable (but not remove) the default source.
    let service = SourcesService::new();
...
fn test_default_source_can_be_re_enabled() {
    // Verifies that a disabled default source can be re-enabled.
    let service = SourcesService::new();
...
fn test_default_source_slug_is_normalized() {
    // Verifies slug normalization (lowercase, alphanumeric, and hyphens).
    let url = Url::parse("https://github.com/athola/gemini-marketplace").unwrap();
